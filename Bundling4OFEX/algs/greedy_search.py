...
def greedy_cpdp(depots, requests, precise=True, seed=1234):
    ...
    while True:
        ...
        best_cost = dists[zero_to_bsz, best_i] # (B,)

        # 5) commit updates **vectorized**
        Q += loads[zero_to_bsz, best_i]

        # 5a) pickups
        pic_mask = (best_i >= D) & (best_i < D+R)       # (B,)
        if pic_mask.any():
            pic_idx = best_i[pic_mask] - D               # which request
            # mark them undelivered
            undel[pic_mask, pic_idx] = True  
            # update load Q (if you need to test capacity)
            # loads = requests[pic_mask, pic_idx, 4]        # (num_pic,)
            # Q[pic_mask,0] += loads
        ...
        if del_mask.any():
            # for b in range(B):
                # if del_mask[b] and [routes[b][-1]] in last_path[b]:
                #     best_i[b] = last_path[b][routes[b][-1]][1]
                # assert last_path[b][routes[b][-1]][1] == best_i[b], f"greedy next nodes is {best_i[b]}, shortest path is {last_path[b][routes[b][-1]]}"
            del_idx = best_i[del_mask] - D - R
            undel[del_mask, del_idx] = False
            # loads = requests[del_mask, del_idx, 4]
            # Q[del_mask,0] += loads
        ...
        for b in range(B):
            try:
                if path[b] and best_i[b] < D + R and best_i[b] >= D:
                    assert best_i[b].item() in path[b], f"greedy next nodes is {best_i[b]}, shortest path is {path[b]}"
            except:
                None
            # even if end, we'll break next loop
    ...
    return routes, L
    # return two_opt_improve(routes, depots, requests)

def multi_start_greedy_cpdp(depots, requests, starting_nodes=10, precise=True):
    """
    Batched greedy PDP over B examples simultaneously.
